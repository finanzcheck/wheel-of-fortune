...
WheelDirective.$inject = ['participantsService', 'winnersService', '$timeout', '$q'];
function WheelDirective(participantsService, winnersService, $timeout, $q) {
    ...
    function WheelDirectiveController (scope) {
        ...
        function init() {
            ...
            wheel = new Raphael(wheelContainer, 800, 800).wheelOfFortune(scope, 400, 400, 250, scope.participants, '#fff', function (a) { angles = a; });
    ...
    function link (scope, element, attr) {
        ...
        function tap() {
            rotate(Math.abs(3600 + chance.integer({min: 0, max: 2}) * 360 + chance.integer({min: 0, max: 360})) * (chance.bool() ? 1 : -1));
        }
    ...
    var paper = this,
        ...
        process = function (participant) {
            var angleplus = 360 * participant.spokes / total,
                ...
                spoke = sector(cx, cy, r, angle, angle + angleplus, {fill: "90-" + bcolor + "-" + color, stroke: stroke, 'stroke-width': 1}),
                txt = paper.text(cx + (r + delta + 55) * Math.cos(-popangle * rad), cy + (r + delta + 25) * Math.sin(-popangle * rad), [participant.name, '|', participant.spokes].join(' ')).attr({fill: bcolor, stroke: 'none', opacity: 0, 'font-size': 20});
